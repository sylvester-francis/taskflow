...
class TestDatabase:
    ...
    def test_user_table_constraints(self):
        ...
        user3 = User(username="testuser2", email="test1@example.com", hashed_password="hash3")
        session.add(user3)
        with pytest.raises(IntegrityError):
            session.commit()
    ...
    def test_cascade_delete_behavior(self):
        ...
        try:
            ...
            remaining_tasks = session.query(Task).filter(Task.owner_id == user.id).count()
            assert remaining_tasks == 0, "Tasks should be cascade deleted when user is deleted"
...
class TestDatabasePerformance:
    ...
    def test_bulk_operations(self):
        ...
        # Bulk update
        session.query(Task).filter(Task.owner_id == user.id).update({"priority": "high"})
        session.commit()
        
        # Verify update
        high_priority_count = session.query(Task).filter(
