...
class TestTaskEndpoints:
    ...
    def test_update_other_user_task(self, client, db_session, test_task):
        ...
        response = client.put(f"/api/tasks/{test_task.id}", json=update_data, headers=headers)
        
        assert response.status_code == 404  # Should not find task owned by different user
